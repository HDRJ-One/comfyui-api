...
def open_websocket_connection():
    """
    Establishes a websocket connection to ComfyUI running under the given address and returns the connection object.
    """
    print("Attempting to connect to WebSocket...")  # Debugging statement
    ws = websocket.create_connection("ws://127.0.0.1:8188/ws")  # Update to include the WebSocket endpoint
    return ws  # Ensure this returns only the WebSocket object
...
def generate_image_by_prompt(prompt, output_path, save_previews=False):
    ...
    try:
        ...
        prompt_id = queue_prompt(prompt, str(uuid.uuid4()), '127.0.0.1:8188')['prompt_id']
        track_progress(prompt, ws, prompt_id)
        images = get_images(prompt_id, '127.0.0.1:8188', save_previews)
...
def generate_image_by_prompt_and_image(prompt, output_path, input_path, filename, save_previews=False):
    ...
    try:
        ws = open_websocket_connection()
        upload_image(input_path, filename, '127.0.0.1:8188')
        prompt_id = queue_prompt(prompt, str(uuid.uuid4()), '127.0.0.1:8188')['prompt_id']
        track_progress(prompt, ws, prompt_id)
        images = get_images(prompt_id, '127.0.0.1:8188', save_previews)
        save_image(images, output_path, save_previews)
...
def generate_image_by_prompt(positive_prompt, negative_prompt, image_path, output_path, save_previews=False):
    """
    Generates an image based on the provided prompts and saves it to the specified output path.

    Args:
        positive_prompt (str): The main text prompt for image generation.
        negative_prompt (str): The text prompt to avoid certain aspects in the image.
        image_path (str): The path to the input image, if applicable.
        output_path (str): The path where the generated image should be saved.
        save_previews (bool): Flag to indicate whether to save preview images.

    Returns:
        dict: Contains information about the generated image or an error message.
    """
    try:
        # Call the main function from gen.py
        main(positive_prompt, negative_prompt, image_path)
        return {"status": "success", "message": "Image generation started!"}
    except Exception as e:
